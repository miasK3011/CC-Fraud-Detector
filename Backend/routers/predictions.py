...
async def predict_random_forest_undersampling(
    data: TransactionData, threshold: float = 0.8596
):
    """
...
async def predict_random_forest_oversampling(
    data: TransactionData, threshold: float = 0.3586
):
    """
...
@router.post(
    ...
    response_model=PredictionResult,
    tags=["XGBoost"],
    summary="Predição de fraude usando modelo XGBoost com undersampling",
)
async def predict_random_forest_oversampling(
    data: TransactionData, threshold: float = 0.9972
):
    """
    Realiza a predição de fraude em transações usando um modelo XGBoost com undersampling.
    """
    model = loadModel("saved_models/xgb_undersampling_model.pkl")
    ...
    try:
        ...
        return {
            "model_name": "Random Forest Undersampling SMOTE",
            "accuracy": 0.80,
            "precision": 0.73,
            "recall": 0.76,
            "f1_score": 0.74,
...
@router.post(
    ...
    response_model=PredictionResult,
    tags=["XGBoost"],
    summary="Predição de fraude usando modelo XGBoost com oversampling SMOTE",
)
async def predict_random_forest_oversampling(
    data: TransactionData, threshold: float = 0.9972
):
    ...
    Realiza a predição de fraude em transações usando um modelo XGBoost com undersampling.
    """
    model = loadModel("saved_models/xgb_oversampling_model.pkl")
